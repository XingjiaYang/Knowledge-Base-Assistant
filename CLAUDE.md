...
- **`app/vector_store.py`** — Markdown-aware chunking (text/code/table separately, heading metadata preserved as `h1`/`h2`/`h3` payload), SentenceTransformers Jina embeddings v5 with `retrieval` task plus `query`/`document` prompts and `classification` task, BM25 keyword indexing over local Markdown chunks, RRF fusion, and Qdrant collection management. Incremental ingest replaces all chunks by `source` filename so no stale chunks accumulate on edits.
